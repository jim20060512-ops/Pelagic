  ...
  return (
    <section className="explore-view">
      ...
      {!visibleLogs.length ? <Empty following={feed === "following"} loggedIn={loggedIn} login={login} /> : <div className="log-grid">{visibleLogs.map((x) => <article className="sighting-card" key={x.id}>
        ...
        <div className="sighting-copy sighting-copy-link" onClick={(event) => { if (!event.target.closest("button")) openDetail(x); }}><p className="card-date">{x.date}</p><h3>{x.species}</h3><p className="site"><MapPin size={14} />{x.locationName}</p>
...
function PhotoViewer({ photos, initialIndex, onClose }) {
  ...
  useEffect(() => { const onKeyDown = (event) => { if (event.key === "Escape") onClose(); if (event.key === "ArrowLeft") previous(); if (event.key === "ArrowRight") next(); }; window.addEventListener("keydown", onKeyDown); return () => window.removeEventListener("keydown", onKeyDown); }, [photos.length]);
  const download = async () => { setDownloading(true); try { const response = await fetch(photo.photo_url); const file = await response.blob(); const link = document.createElement("a"); const objectUrl = URL.createObjectURL(file); link.href = objectUrl; link.download = `${(photo.species || "pelagic-dive-photo").replace(/[\\/:*?\"<>|]/g, "-")}.jpg`; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(objectUrl); } finally { setDownloading(false); } };
  return <div className="photo-viewer" role="dialog" aria-modal="true" aria-label="照片檢視器"><div className="photo-viewer-bar"><span>{index + 1} / {photos.length}{photo.species ? ` · ${photo.species}` : ""}</span><div><button onClick={download} disabled={downloading}><Download />{downloading ? "準備中…" : "下載"}</button><button aria-label="關閉照片檢視器" onClick={onClose}><X /></button></div></div><img src={photo.photo_url} alt={`${photo.species || "未命名生物"} 的水下照片`} />{photos.length > 1 && <><button className="viewer-nav previous" aria-label="上一張" onClick={previous}><ChevronLeft /></button><button className="viewer-nav next" aria-label="下一張" onClick={next}><ChevronRight /></button></>}</div>;
}
...
function ProfilePage({ currentUser, profile, setProfile, owner, ownLogs, setNotice, following, toggleFollow, openMap, openDetail }) {
