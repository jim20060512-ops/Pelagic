...
export async function deleteDiveComment({ commentId, userId }) {
  const { error } = await requireSupabase().from('dive_log_comments').delete().eq('id', commentId).eq('user_id', userId)
  if (error) throw error
}
